...
public class EventConsumer {
  ...
  private final KafkaConsumer<Long, ProductEvent> consumer;
  ...
  public EventConsumer() {
    ...
    consumer = new KafkaConsumer<Long, ProductEvent>(kafkaProperties);
    consumer.subscribe(Arrays.asList(EventConfig.PRODUCT_TOPIC));
    consumer.poll(Duration.ofMillis(100));
  }

  public List<ProductEvent> consume() {
    List<ProductEvent> consumedEvents = new ArrayList<>();
    int attempts = 0;
    while (attempts < 100) {
      attempts++ ;
      consumer.seekToBeginning(consumer.assignment());
      ConsumerRecords<Long, ProductEvent> records = consumer.poll(Duration.ofMillis(100));
      for (ConsumerRecord<Long, ProductEvent> record : records) {
