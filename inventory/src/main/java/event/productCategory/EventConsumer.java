...
public class EventConsumer {
  ...
  private final KafkaConsumer<Long, CategoryEvent> consumer;
  ...
  public EventConsumer() {
    ...
    consumer = new KafkaConsumer<Long, CategoryEvent>(kafkaProperties);
    consumer.subscribe(Arrays.asList(EventConfig.CATEGORY_TOPIC));
    consumer.poll(Duration.ofMillis(100));
  }

  public List<CategoryEvent> consume() {
    List<CategoryEvent> consumedEvents = new ArrayList<>();
    int attempts = 0;
    while (attempts < 100) {
      attempts++ ;
      consumer.seekToBeginning(consumer.assignment());
      ConsumerRecords<Long, CategoryEvent> records = consumer.poll(Duration.ofMillis(100));
      for (ConsumerRecord<Long, CategoryEvent> record : records) {
